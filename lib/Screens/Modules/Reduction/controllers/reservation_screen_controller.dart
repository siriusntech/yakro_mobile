...
class ReservationScreenController extends GetxController {
  ...
  Rx<DateTimeRange> dateRangeController = DateTimeRange(start: DateTime.now(), end: DateTime.now()).obs;
  ...
  final ReservationApiController reservationApiController = Get.put(ReservationApiController());
  final HotelSingleScreenController hotelSingleScreenController = Get.find<HotelSingleScreenController>();
  ...
  void onInit() {
    ...
    dateRangeController.value = DateTimeRange(start: DateTime.now(), end: DateTime.now());
  ...
  Future<void> storeReservation() async {
    ...
    if(data['nom'] == '' || data['prenom'] == '' || data['telephone'] == '' || data['date_arrivee'] == '' || data['date_depart'] == '' || data['nbr_chambre'] == ''){
      Get.snackbar('Erreur', 'Veuillez remplir tous les champs', backgroundColor: ConstColors.alertWarnig, colorText: Colors.white, icon: const Icon(Icons.warning, color: Colors.white,));
      return;
    }
  ...
  void setDateRange(DateTimeRange dateRange){
    ...
    getTotal();
    dateArrivee.value = '${dateRange.end.day}-${dateRange.end.month}-${dateRange.end.year}';
    dateDepart.value = '${dateRange.start.day}-${dateRange.start.month}-${dateRange.start.year}';
    update();
  }
  ...
  double getTotal(){
    getTotalaPayer();
    totalController.value.text = double.parse(((int.parse(typeChambreHotel!.prix.toString()) * int.parse(nbrChambre.value.text))*dateRangeController.value.duration.inDays).toString()).toString();
    // print(totalController.value.text);
    update();
    return double.parse(((int.parse(typeChambreHotel!.prix.toString()) * int.parse(nbrChambre.value.text))*dateRangeController.value.duration.inDays).toString());

  }

  double getTotalaPayer(){
    update();
    return dateRangeController.value.duration.inDays==0?0.0: double.parse(totalController.value.text)- (double.parse(totalController.value.text)* double.parse(hotelSingleScreenController.hotelModel.value.reduction.toString())/100);

  }

  double getReduction(){
    update();
    return double.parse(hotelSingleScreenController.hotelModel.value.reduction.toString());
  }
